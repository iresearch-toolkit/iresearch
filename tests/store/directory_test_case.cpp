 ...
 public:
  static void smoke_store(directory& dir) {
    ...
    for (const auto& name : names) {
      ...
      file->write_bytes(reinterpret_cast<const byte_type*>(it->c_str()),
                        it->size());
    ...
    for (const auto& name : names) {
...

      {
        ...
        ASSERT_EQ(ref_cast<byte_type>(std::string_view(*it)), buf);
        ...
        // random access
        {
          ...
          ASSERT_EQ(read, it->size());
          ASSERT_EQ(ref_cast<byte_type>(std::string_view(*it)), buf1);
        ...
        // failed direct buffer access doesn't move file pointer
        {
          ...
          ASSERT_EQ(nullptr,
                    file->read_buffer(file->length() - 1, file->length(),
                                      BufferHint::NORMAL));
          ASSERT_EQ(fp, file->file_pointer());
        }
    ...
    // Check read_bytes after the end of file
    {
      ...
      // read from file
      {
        ...
        expected =
          in->length() - sizeof buf;  // 'sizeof buf' already read above
...
TEST_P(directory_test_case, smoke_index_io) {
  ...
  const bstring payload(iresearch::ref_cast<byte_type>(std::string_view(name)));
...
TEST_P(directory_test_case, smoke_store) { smoke_store(*dir_); }
...
TEST(memory_directory_test, rewrite) {
  ...
  const bytes_view payload0{ref_cast<byte_type>(str0)};
  const bytes_view payload1{ref_cast<byte_type>(str1)};
