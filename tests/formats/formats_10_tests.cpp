...
class format_10_test_case : public tests::format_test_case_base {
 protected:
  ...
  static const size_t INDEX_BLOCK_SIZE = 1024;
  static const size_t MAX_DATA_BLOCK_SIZE = 4096;

  class dense_mask_column {
   public:
    bool read(irs::index_input& in) {
      const auto items_per_block = in.read_vlong();
      const auto count = in.read_vlong();
      const auto max = in.read_vlong();
      const auto doc_blocks_count = std::ceil(double(count) / items_per_block);
      const auto index_blocks_count = std::ceil(doc_blocks_count/INDEX_BLOCK_SIZE);

      for (size_t size = in.read_vlong(); size; size = in.read_vlong()) {
        irs::encode::avg::skip_block64(in, size); // skip keys
        irs::encode::avg::skip_block64(in, size); // skip offsets
      }
   ...
   private:
    irs::doc_id_t min_{ irs::type_limits<irs::type_t::doc_id_t>::eof() };
    irs::doc_id_t max_{ irs::type_limits<irs::type_t::doc_id_t>::eof() };
  }; // dense_mask_column

  class sparse_mask_column {
   public:
    bool read(irs::data_input& in) {
      const auto items_per_block = in.read_vlong();
      const auto count = in.read_vlong();
      const auto doc_blocks_count = std::ceil(double(count) / items_per_block);
      const auto index_blocks_count = std::ceil(doc_blocks_count/INDEX_BLOCK_SIZE);
    ...
    bool value(
        irs::doc_id_t key,
        const irs::columnstore_reader::value_reader_f& /*reader*/) const {
      return docs_.test(key);
    };

   private:
    iresearch::bitset docs_;
  }; // sparse_mask_column
  ...
  class dense_fixed_length_block : irs::util::noncopyable {
   public:
    bool load(irs::index_input& in, irs::decompressor& decomp, irs::bstring& buf) {
      ...
      if (!irs::encode::avg::check_block_rl64(in, 1)) {
        // invalid block type
        return false;
      }

      if (!irs::encode::avg::read_block_rl64(in, base_, length_)) {
        // invalid block type
        return false;
      }

      //read_compact(in, MAX_DATA_BLOCK_SIZE, decomp, buf, data_);
    ...
    bool value(
      ...
      const auto vstart = base_ + key*length_;

      irs::bytes_ref_input stream;
      stream.reset(data_.c_str() + vstart, length_);

      return reader(stream);
    }

   private:
    uint64_t base_{}; // base offset
    uint64_t length_{}; // entry length
  ...
  class dense_block : irs::util::noncopyable {
   public:
    ...
    bool value(
      ...
      const auto vstart = index_[key];
      const auto vend = (++key == size_ ? data_.size() : index_[key]);
      assert(vend >= vstart);

      irs::bytes_ref_input stream;
      stream.reset(data_.c_str() + vstart, vend - vstart);
  ...
  template<typename Allocator = std::allocator<dense_block>>
  class read_context
   ...
   public:
    read_context(const Allocator& alloc = Allocator())
    ...
    Block* emplace_back(uint64_t offset, Args&&... args) {
      ...
      if (!block.load(*stream_, decomp_, buf_)) {
   ...
   private:
    ...
    irs::decompressor decomp_; // decompressor
    irs::bstring buf_; // temporary buffer for decoding/unpacking
    irs::index_input::ptr stream_;
  }; // read_context

  template<typename Context, typename Block>
  static const Block* load_block(
      Context& ctx,
      uint64_t offset,
  ...
  class dense_column {
   public:
    typedef dense_block block_t;

    bool read(irs::data_input& in) {
      uint64_t buf[INDEX_BLOCK_SIZE]; // buffer for unpacking

      const auto items_per_block = in.read_vlong();
      assert(items_per_block);
      const auto count = in.read_vlong();
      const auto max = in.read_vlong();
      const auto doc_blocks_count = std::ceil(double(count) / items_per_block);

      std::vector<block_ref> refs(doc_blocks_count);

      auto begin = refs.begin();
      for (size_t size = in.read_vlong(); size; begin += size, size = in.read_vlong()) {
        if (!irs::encode::avg::check_block_rl64(in, items_per_block)) {
          // invalid column type
          return false;
        }
      ...
      min_ = max - count;
      max_ = count;
      items_per_block_ = items_per_block;
    ...
    bool value(
      ...
      const auto block_idx = key / items_per_block_;
      ...
      assert(cached);
      return cached->value(key -= block_idx*items_per_block_, reader);
    }
   ...
   private:
    ...
    irs::doc_id_t max_; // min key
    irs::doc_id_t min_; // max key
    irs::doc_id_t items_per_block_; // number of items in a single block
  ...
  class dense_fixed_length_column {
   public:
    ...
    bool read(irs::data_input& in) {
      ...
      std::vector<index_block> blocks(index_blocks_count);
      auto begin = blocks.begin();
      for (size_t size = in.read_vlong(); size; size = in.read_vlong(), ++begin) {
        size_t base, avg;
        if (!irs::encode::avg::check_block_rl64(in, items_per_block)) {
          // invalid column type
          return false;
        ...
        if (!irs::encode::avg::read_block_rl64(in, begin->base, begin->avg)) {
          // invalid column type
          return false;
        }
      }

      refs_ = std::vector<block_ref>(doc_blocks_count);
    ...
    bool value(
      ...
      const auto idx = key / items_per_block_;
      const auto block_idx = idx / INDEX_BLOCK_SIZE;

      const auto& block = blocks_[block_idx];
      const auto offset = block.base + block.avg*(idx%INDEX_BLOCK_SIZE);

      auto& ref = const_cast<block_ref&>(refs_[idx]);
      const auto* cached = ref.load();

      if (!cached) {
        read_context<> ctx;

        if (!(cached = load_block(ctx, offset, ref))) {
          // unable to load block
          return false;
        }
      }
      ...
      assert(cached);
      return cached->value(key -= idx*items_per_block_, reader);
    };
   ...
   private:
    ...
    typedef std::atomic<const block_t*> block_ref;

    irs::doc_id_t min_; // min key
    irs::doc_id_t max_; // max key
    irs::doc_id_t items_per_block_;
    std::vector<block_ref> refs_;
    std::vector<index_block> blocks_;
  }; // dense_fixed_length_column

//  void dense_fixed_length_column_read_write() {
...
//      out->write_vlong(block_docs);
//      out->write_vlong(blocks_count*block_docs + last_block_docs_count);
//      out->write_vlong(blocks_count*block_docs + last_block_docs_count);
//      ASSERT_FALSE(!out);
...
//      dense_fixed_length_column column;
//      ASSERT_TRUE(column.read(*in));
//
//      auto hh = [](irs::data_input&) { return false; };
//
//      auto b = column.value(128, hh);
//      b = column.value(0, hh);
//      b = column.value(132, hh);
//      int i = 45;
//      b = column.value(650000, hh);
//    }
//  }
//
//  void sparse_column_read_write() {
//    const size_t start_offset = 100;
//    const ir::doc_id_t blocks_count = 5000;
//    const ir::doc_id_t block_docs = 128;
//    const ir::doc_id_t last_block_docs_count = 73;
...
//  }
//  
//  void dense_mask_column_read_write() {
//    const size_t start_offset = 100;
//    const ir::doc_id_t blocks_count = 5000;
//    const ir::doc_id_t block_docs = 128;
//    const ir::doc_id_t last_block_docs_count = 73;
