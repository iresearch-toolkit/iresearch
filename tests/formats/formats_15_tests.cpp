...

  {
...

    {
      ...
      auto assert_docs_seq = [&]() {
        ...
        auto actual =
            reader->wanderator(field.index_features, features, factory, read_meta);
      ...
      // next + seek to eof
      {
        ...
        auto it = reader->wanderator(field.index_features, irs::IndexFeatures::NONE, factory, read_meta);
      ...
      // seek to irs::doc_limits::invalid()
      {
        ...
        auto it = reader->wanderator(field.index_features,
                                     irs::IndexFeatures::NONE, factory, read_meta);
        ASSERT_FALSE(irs::doc_limits::valid(it->value()));
        ASSERT_FALSE(
