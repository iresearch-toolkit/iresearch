...
protected:
  ...
  void all_order() {
    ...
    // custom order
    {
      ...
      sort.collector_term = [&collector_term_count](const irs::attribute_store&)->void { ++collector_term_count; };
    ...
    // custom order (no scorer)
    {
      ...
      sort.prepare_scorer = [](const irs::sub_reader&, const irs::term_reader&, const irs::attribute_store&, const irs::attribute_store&)->irs::sort::scorer::ptr { return nullptr; };
