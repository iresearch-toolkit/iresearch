 ...
 protected:
  ...
  void all_order() {
    ...
    // custom order (no scorer)
    {
      ...
      sort.prepare_scorer = [](const irs::sub_reader&, const irs::term_reader&, const irs::byte_type*, const irs::attribute_provider&)->std::pair<irs::score_ctx_ptr, irs::score_f> { return { nullptr, nullptr }; };
