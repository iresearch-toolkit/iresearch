    ...
    tests::json_doc_generator gen(
        ...
        [](tests::document& doc, const std::string& name,
          ...
          if (data.is_string()) {  // field
            ...
            const auto value = std::to_string(data.as_number<uint64_t>());
            doc.insert(std::make_shared<string_field>(name, value), false,
                       true);
          }
        });
    add_segment(gen);
...
TEST_P(tfidf_test_case, test_collector_serialization) {
  ...
  // default init (field_collector)
  {
    ...
    ASSERT_TRUE(
        out0.out_.size() != out1.out_.size() ||
        0 != std::memcmp(&out0.out_[0], &out1.out_[0], out0.out_.size()));
    ...
    ASSERT_TRUE(
        out0.out_.size() == out2.out_.size() &&
        0 == std::memcmp(&out0.out_[0], &out2.out_[0], out0.out_.size()));
    ...
    ASSERT_TRUE(
        out1.out_.size() == out3.out_.size() &&
        0 == std::memcmp(&out1.out_[0], &out3.out_[0], out1.out_.size()));
  }
