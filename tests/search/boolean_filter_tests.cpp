...
TEST(basic_disjunction, next) {
  using disjunction = irs::basic_disjunction<irs::doc_iterator::ptr, irs::NoopAggregator>;
  // simple case
  {
...
TEST(basic_disjunction_test, scored_seek_next) {
    ...
    auto it_ptr = irs::ResoveMergeType(
       ...
       [&]<typename A>(A&& aggregator) -> irs::doc_iterator::ptr {
         ...
         return irs::memory::make_managed<disjunction>(
           adapter(irs::memory::make_managed<detail::basic_doc_iterator>(first.begin(), first.end(), empty_stats, prepared_first_order)),
           adapter(irs::memory::make_managed<detail::basic_doc_iterator>(last.begin(), last.end(), empty_stats, prepared_last_order)),
           std::move(aggregator));
       });
  ...
  // disjunction with order, aggregate scores
  {
    ...
    auto prepared_first_order = irs::Order::Prepare(first_order);
    ...
    ASSERT_TRUE(it.next());
    ASSERT_EQ(1, it.value());
    ASSERT_EQ(3, *score.evaluate()); // 1 + 2
    ASSERT_EQ(5, it.seek(5));
    ASSERT_EQ(3, *score.evaluate()); // 1 + 2
    ASSERT_TRUE(it.next());
    ASSERT_EQ(2, *score.evaluate()); // 2
    ASSERT_EQ(6, it.value());
    ASSERT_TRUE(it.next());
    ASSERT_EQ(7, it.value());
    ASSERT_EQ(1, *score.evaluate()); // 1
    ASSERT_EQ(11, it.seek(10));
    ASSERT_EQ(1, *score.evaluate()); // 1
    ASSERT_TRUE(it.next());
    ASSERT_EQ(45, it.value());
  ...
  // disjunction with order, max score
  {
    ...
    ASSERT_TRUE(it.next());
    ASSERT_EQ(1, it.value());
    ASSERT_EQ(2, *score.evaluate()); // std::max(1, 2)
    ASSERT_EQ(5, it.seek(5));
    ASSERT_EQ(2, *score.evaluate()); // std::max(1, 2)
    ASSERT_TRUE(it.next());
    ASSERT_EQ(2, *score.evaluate()); // std::max(2)
    ASSERT_EQ(6, it.value());
    ASSERT_TRUE(it.next());
    ASSERT_EQ(7, it.value());
    ASSERT_EQ(1, *score.evaluate()); // std::max(1)
    ASSERT_EQ(11, it.seek(10));
    ASSERT_EQ(1, *score.evaluate()); // std::max(1)
    ASSERT_TRUE(it.next());
    ASSERT_EQ(45, it.value());
  ...
  // disjunction with order, iterators without order, aggregation
  {
    ...
    auto it_ptr = irs::ResoveMergeType(
      ...
      [&]<typename A>(A&& aggregator) -> irs::doc_iterator::ptr {
        ...
        return irs::memory::make_managed<disjunction>(
          adapter(irs::memory::make_managed<detail::basic_doc_iterator>(first.begin(), first.end(), empty_stats)),
          adapter(irs::memory::make_managed<detail::basic_doc_iterator>(last.begin(), last.end(), empty_stats)),
          std::move(aggregator));
      });
...
TEST(small_disjunction_test, next) {
  using disjunction = irs::small_disjunction<irs::doc_iterator::ptr, irs::NoopAggregator>;
...
TEST(small_disjunction_test, scored_seek_next) {
  ...
  // disjunction with score, sub-iterators with scores AGGREGATED score
  {
    ...
    ASSERT_TRUE(it.next());
    ASSERT_EQ(1, it.value());
    ASSERT_EQ(7, *score.evaluate()); // 1+2+4
    ASSERT_EQ(5, it.seek(5));
    ASSERT_EQ(7, *score.evaluate()); // 1+2+4
    ASSERT_TRUE(it.next());
    ASSERT_EQ(6, it.value());
    ASSERT_EQ(6, *score.evaluate()); // 2+4
    ASSERT_TRUE(it.next());
    ASSERT_EQ(7, it.value());
    ASSERT_EQ(1, *score.evaluate()); // 1
    ASSERT_EQ(29, it.seek(27));
  ...
  // disjunction with score, sub-iterators with scores, MAX score
  {
    ...
    ASSERT_TRUE(it.next());
    ASSERT_EQ(1, it.value());
    ASSERT_EQ(4, *score.evaluate()); // std::max(1, 2, 4)
    ASSERT_EQ(5, it.seek(5));
    ASSERT_EQ(4, *score.evaluate()); // std::max(1, 2, 4)
    ASSERT_TRUE(it.next());
    ASSERT_EQ(6, it.value());
    ASSERT_EQ(4, *score.evaluate()); // std::max(2, 4)
    ASSERT_TRUE(it.next());
    ASSERT_EQ(7, it.value());
    ASSERT_EQ(1, *score.evaluate()); // std::max(1)
    ASSERT_EQ(29, it.seek(27));
  ...
  // disjunction with score, sub-iterators partially with scores, aggregation
  {
    ...
    ASSERT_TRUE(it.next());
    ASSERT_EQ(1, it.value());
    ASSERT_EQ(5, *score.evaluate()); // 1+4
    ASSERT_EQ(5, it.seek(5));
    ASSERT_EQ(5, *score.evaluate()); // 1+4
    ASSERT_TRUE(it.next());
    ASSERT_EQ(6, it.value());
    ASSERT_EQ(4, *score.evaluate()); // 4
    ASSERT_TRUE(it.next());
    ASSERT_EQ(7, it.value());
    ASSERT_EQ(1, *score.evaluate()); // 1
    ASSERT_EQ(29, it.seek(27));
  ...
  // disjunction with score, sub-iterators partially with scores, max scores
  {
    ...
    ASSERT_TRUE(it.next());
    ASSERT_EQ(1, it.value());
    ASSERT_EQ(4, *score.evaluate()); // std::max(1, 4)
    ASSERT_EQ(5, it.seek(5));
    ASSERT_EQ(4, *score.evaluate()); // std::max(1, 4)
    ASSERT_TRUE(it.next());
    ASSERT_EQ(6, it.value());
    ASSERT_EQ(4, *score.evaluate()); // std::max(4)
    ASSERT_TRUE(it.next());
    ASSERT_EQ(7, it.value());
    ASSERT_EQ(1, *score.evaluate()); // std::max(1)
    ASSERT_EQ(29, it.seek(27));
    ASSERT_EQ(0, *score.evaluate()); // default value
    ASSERT_TRUE(it.next());
    ASSERT_EQ(45, it.value());
    ASSERT_EQ(1, *score.evaluate()); // std::max(1)
    ASSERT_FALSE(it.next());
  ...
  // disjunction with score, sub-iterators partially without scores, aggregation
  {
    ...
    ASSERT_TRUE(it.next());
    ASSERT_EQ(1, it.value());
    ASSERT_EQ(0, *score.evaluate()); // 1+4
    ASSERT_EQ(5, it.seek(5));
    ASSERT_EQ(0, *score.evaluate()); // 1+4
    ASSERT_TRUE(it.next());
    ASSERT_EQ(6, it.value());
    ASSERT_EQ(0, *score.evaluate()); // 4
    ASSERT_TRUE(it.next());
    ASSERT_EQ(7, it.value());
    ASSERT_EQ(0, *score.evaluate()); // 1
    ASSERT_EQ(29, it.seek(27));
    ASSERT_EQ(0, *score.evaluate()); //
    ASSERT_TRUE(it.next());
    ASSERT_EQ(45, it.value());
...
TEST(block_disjunction_test, next_scored) {
  ...
  // disjunction score, sub-iterators with scores
  {
                                     ...
                                     irs::Order::Prepare(detail::basic_sort{1})));
    {
      ...
      for (; it.next();) {
        ASSERT_EQ(doc->value, it.value());
        result.emplace_back(it.value(), *score.evaluate());
        ASSERT_EQ(1, it.match_count());
      }
      ASSERT_EQ(0, it.match_count());
      ASSERT_TRUE(irs::doc_limits::eof(it.value()));
  ...
  {
    std::vector<std::pair<irs::doc_id_t, irs::score_t>> expected{
      { 1, 2.f }, { 2.f, 2.f }, { 5, 2.f }, { 7, 2.f }, { 9, 2.f }, { 11, 2.f },
      { 1145, 2.f }, { 111165, 2.f }, { 1111178, 2.f }, { 111111127, 2.f }};
    std::vector<std::pair<irs::doc_id_t, irs::score_t>> result;
...
TEST(block_disjunction_test, next_scored_two_blocks) {
  auto order = [](size_t i) -> irs::Order {
    return irs::Order::Prepare(detail::basic_sort{i});
  };
...
TEST(block_disjunction_test, seek_scored_no_readahead) {
  ...
  auto it_ptr = irs::ResoveMergeType(
    ...
    for (const auto& target : expected) {
      ...
      ASSERT_EQ(doc->value, it.value());
      ASSERT_EQ(target.match_count, it.match_count());
      ASSERT_EQ(target.score, *score.evaluate());
    }
  }
...
TEST(disjunction_test, next) {
  using disjunction = irs::disjunction<irs::doc_iterator::ptr, irs::NoopAggregator>;
  auto sum = [](size_t sum, const std::vector<irs::doc_id_t>& docs) { return sum += docs.size(); };
...
TEST(min_match_disjunction_test, next) {
  using disjunction = irs::min_match_disjunction<irs::doc_iterator::ptr, irs::NoopAggregator>;
  // single dataset
  {
    std::vector<std::vector<irs::doc_id_t>> docs{
        { 1, 2, 5, 7, 9, 11, 45 },
...
TEST(min_match_disjunction_test, seek) {
  ...
  // empty datasets
  {
    ...
    {
      disjunction it(detail::execute_all<disjunction::cost_iterator_adapter>(docs), 0);
      auto* doc = irs::get<irs::document>(it);
      ASSERT_TRUE(bool(doc));
      for (const auto& target : expected) {
        ASSERT_EQ(target.expected, it.seek(target.target));
    ...
    {
      disjunction it(detail::execute_all<disjunction::cost_iterator_adapter>(docs), 1);
      auto* doc = irs::get<irs::document>(it);
      ASSERT_TRUE(bool(doc));
      for (const auto& target : expected) {
        ASSERT_EQ(target.expected, it.seek(target.target));
...
TEST(min_match_disjunction_test, seek_next) {
...

  {
    ...
    disjunction it(detail::execute_all<disjunction::cost_iterator_adapter>(docs), 2);
    auto* doc = irs::get<irs::document>(it);
    ASSERT_TRUE(bool(doc));
...
TEST(min_match_disjunction_test, scored_seek_next) {
  // disjunction without score, sub-iterators with scores
  {
    ...
    using ExpectedType = irs::min_match_disjunction<irs::doc_iterator::ptr, irs::NoopAggregator>;
    ASSERT_NE(nullptr, dynamic_cast<ExpectedType*>(it_ptr.get()));
    auto& it = dynamic_cast<ExpectedType&>(*it_ptr);
  ...
  // disjunction with score, sub-iterators with scores partially, aggregate
  {
    ...
    ASSERT_TRUE(it.next());
    ASSERT_EQ(1, it.value());
    ASSERT_EQ(5, *score.evaluate()); // 1+2+4
    ASSERT_EQ(5, it.seek(5));
    ASSERT_EQ(5, *score.evaluate()); // 1+2+4
    ASSERT_TRUE(it.next());
    ASSERT_EQ(6, it.value());
    ASSERT_EQ(4, *score.evaluate()); // 2+4
    ASSERT_TRUE(it.next());
    ASSERT_EQ(9, it.value());
    ASSERT_EQ(5, *score.evaluate()); // 1+4
    ASSERT_EQ(29, it.seek(27));
  ...
  // disjunction with score, sub-iterators without scores, aggregate
  {
    ...
    ASSERT_TRUE(it.next());
    ASSERT_EQ(1, it.value());
    ASSERT_EQ(0, *score.evaluate()); // 1+2+4
    ASSERT_EQ(5, it.seek(5));
    ASSERT_EQ(0, *score.evaluate()); // 1+2+4
    ASSERT_TRUE(it.next());
    ASSERT_EQ(6, it.value());
    ASSERT_EQ(0, *score.evaluate()); // 2+4
    ASSERT_TRUE(it.next());
    ASSERT_EQ(9, it.value());
    ASSERT_EQ(0, *score.evaluate()); // 1+4
    ASSERT_EQ(29, it.seek(27));
...
TEST(conjunction_test, next) {
  using conjunction = irs::conjunction<irs::doc_iterator::ptr, irs::NoopAggregator>;
...
TEST(conjunction_test, scored_seek_next) {
  ...
  // conjunction with 4 sub-iterators with score, sub-iterators with scores, aggregation
  {
    ...
    ASSERT_TRUE(it.next());
    ASSERT_EQ(1, it.value());
    ASSERT_EQ(12, *score.evaluate()); // 1+2+4+5
    ASSERT_EQ(4, it.seek(3));
    ASSERT_EQ(12, *score.evaluate()); // 1+2+4+5
    ASSERT_TRUE(it.next());
    ASSERT_EQ(5, it.value());
  ...
  {
    std::vector<std::pair<std::vector<irs::doc_id_t>, irs::Order>> docs;
    docs.emplace_back(std::vector<irs::doc_id_t>{ 1, 2, 4, 5, 7, 8, 9, 11, 14, 45 }, irs::Order{});
    docs.emplace_back(std::vector<irs::doc_id_t>{ 1, 4, 5, 6, 8, 12, 14, 29 }, irs::Order{});
    docs.emplace_back(std::vector<irs::doc_id_t>{ 1, 4, 5, 8, 14 }, irs::Order{});
...
TEST_P(boolean_filter_test_case, mixed_ordered) {
...

  {
    ...
    std::array<irs::sort::ptr, 2> ord{
      std::make_unique<irs::tfidf_sort>(),
