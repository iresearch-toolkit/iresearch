...
void insert_find_core(const std::vector<std::string>& src, uint32_t block_size) {
  block_pool<byte_type> pool(block_size);
  block_pool<byte_type>::inserter writer(pool.begin());
  postings bh(writer);
...
TEST(postings_tests, insert_find) {
  ...
  tests::insert_find_core(data, 32768);
}
...
TEST(postings_tests, slice_alignment) {
  ...
  // read_write on new block pool
  {
    ...
    block_pool<byte_type>::inserter writer(begin);
    postings bh(writer);
