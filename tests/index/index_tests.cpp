 ...
 public:
  ...
  void clear_writer() {
    ...
    // test creation of an empty writer
    {
      ...
      ASSERT_THROW(irs::directory_reader::open(dir),
  ...
  void writer_check_open_modes() {
    ...
    ASSERT_THROW(irs::directory_reader::open(dir(), codec()),
                 irs::index_not_found);
    ...
    // read columns
    {
      auto visit_column = [&segment](const irs::string_ref& column_name) {
        ...
        auto visitor = [&gen, &column_name, &expected_id](
                           irs::doc_id_t id,
                           const irs::bytes_ref& actual_value) {
          if (id != ++expected_id) {
            return false;
          }
...
TEST_P(index_test_case, document_context) {
  ...
  {
    auto writer = open_writer();
    auto field_cond_lock =
        irs::make_unique_lock(field.cond_mutex);  // wait for insertion to start
  ...
  // during replace across commit blocks (single doc)
  {
    ...
    auto result =
        field.cond.wait_for(field_cond_lock, 100ms);  // verify commit() blocks
  ...
  // during replace across commit blocks (functr)
  {
    ...
    auto field_cond_lock =
        irs::make_unique_lock(field.cond_mutex);  // wait for insertion to start
    // auto field_lock = irs::make_unique_lock(field.mutex); // prevent field
    // from finishing
    field.wait = true;  // prevent field from finishing
  ...
  // holding document_context after remove across commit does not block
  {
    ...
    ASSERT_EQ(std::cv_status::timeout, result);
    field_cond_lock
        .unlock();  // verify commit() finishes FIXME TODO use below once
                    // segment_context will not block flush_all()
    // ASSERT_EQ(std::cv_status::no_timeout, result); // verify commit()
    // finishes
    //  FIXME TODO add once segment_context will not block flush_all()
    // ASSERT_TRUE(commit);
    {
  ...
  // flush_contexts)
  {
    ...
    /* FIXME TODO use below once segment_context will not block flush_all()
        {
          auto ctx = writer->documents(); // will reuse segment_context from
       above
       ...
       segment.mask(termItr->postings(irs::IndexFeatures::DOCS));
          ...
          ASSERT_EQ(docsItr->value(), values->seek(docsItr->value()));
          ASSERT_EQ("A",
       irs::to_string<irs::string_ref>(actual_value->value.c_str())); // 'name'
       value in doc1 ASSERT_FALSE(docsItr->next());
        }

        {
          ...
          ASSERT_EQ("B",
       irs::to_string<irs::string_ref>(actual_value->value.c_str())); // 'name'
       value in doc2 ASSERT_FALSE(docsItr->next());
        }

        {
          ...
          ASSERT_EQ("C",
       irs::to_string<irs::string_ref>(actual_value->value.c_str())); // 'name'
       value in doc3 ASSERT_FALSE(docsItr->next());
        }
    */
  }
  ...
  // fileds
  {
    ...
    writer->documents().insert().insert<irs::Action::STORE>(
        doc2->stored.begin(),
        doc2->stored.end());  // document without any indexed attributes (reuse
                              // segment writer)
    writer->commit();
...
TEST_P(index_test_case, doc_removal) {
  ...
  // new segment: remove + add
  {
    ...
    ASSERT_TRUE(insert(*writer, doc1->indexed.begin(), doc1->indexed.end(),
                       doc1->stored.begin(), doc1->stored.end()));
    writer->documents().remove(
        std::move(query_doc2.filter));  // not present yet
    ASSERT_TRUE(insert(*writer, doc2->indexed.begin(), doc2->indexed.end(),
                       doc2->stored.begin(), doc2->stored.end()));
    writer->commit();
...
TEST_P(index_test_case, doc_update) {
  ...
  // new segment update (as reference)
  {
    ...
    ASSERT_TRUE(insert(*writer, doc1->indexed.begin(), doc1->indexed.end(),
                       doc1->stored.begin(), doc1->stored.end()));
    ASSERT_TRUE(update(*writer, *(query_doc1.filter.get()),
  ...
  // new segment update (as unique_ptr)
  {
    ...
    ASSERT_TRUE(insert(*writer, doc1->indexed.begin(), doc1->indexed.end(),
                       doc1->stored.begin(), doc1->stored.end()));
    ASSERT_TRUE(update(*writer, std::move(query_doc1.filter),
                       doc2->indexed.begin(), doc2->indexed.end(),
                       doc2->stored.begin(), doc2->stored.end()));
    writer->commit();
  ...
  // new segment update (as shared_ptr)
  {
    ...
    ASSERT_TRUE(insert(*writer, doc1->indexed.begin(), doc1->indexed.end(),
                       doc1->stored.begin(), doc1->stored.end()));
    ASSERT_TRUE(update(
        *writer, std::shared_ptr<irs::filter>(std::move(query_doc1.filter)),
  ...
  // 3x updates (same segment)
  {
    ...
    ASSERT_TRUE(update(*writer, std::move(query_doc1.filter),
                       doc2->indexed.begin(), doc2->indexed.end(),
                       doc2->stored.begin(), doc2->stored.end()));
    ASSERT_TRUE(update(*writer, std::move(query_doc2.filter),
                       doc3->indexed.begin(), doc3->indexed.end(),
                       doc3->stored.begin(), doc3->stored.end()));
    ASSERT_TRUE(update(*writer, std::move(query_doc3.filter),
  ...
  // update + delete (same segment)
  {
    ...
    ASSERT_TRUE(insert(*writer, doc2->indexed.begin(), doc2->indexed.end(),
                       doc2->stored.begin(), doc2->stored.end()));
    ASSERT_TRUE(update(*writer, *(query_doc2.filter), doc3->indexed.begin(),
                       doc3->indexed.end(), doc3->stored.begin(),
                       doc3->stored.end()));
    writer->documents().remove(
        *(query_doc2.filter));  // remove no longer existent
    writer->commit();
  ...
  // delete + update then update (2nd - update of modified doc) (same segment)
  {
    ...
    ASSERT_TRUE(update(*writer, *(query_doc2.filter), doc3->indexed.begin(),
                       doc3->indexed.end(), doc3->stored.begin(),
                       doc3->stored.end()));
    ASSERT_TRUE(update(*writer, *(query_doc3.filter), doc4->indexed.begin(),
  ...
  // delete + update then update (2nd - update of modified doc) (different
  // segments)
  {
    auto query_doc2 = irs::iql::query_builder().build("name==B", "C");
    auto query_doc3 = irs::iql::query_builder().build("name==C", "C");
  ...
  // new segment failed update (due to field features mismatch or failed
  // serializer)
  {
    ...
    ASSERT_FALSE(update(*writer, *(query_doc1.filter.get()),
                        doc3->indexed.begin(), doc3->indexed.end(),
                        doc3->stored.begin(), doc3->stored.end()));
    writer->commit();
...
TEST_P(index_test_case, import_reader) {
  ...
  // new: add + add + delete, old: import
  {
    ...
    ASSERT_TRUE(insert(*writer, doc3->indexed.begin(), doc3->indexed.end(),
                       doc3->stored.begin(), doc3->stored.end()));
    writer->documents().remove(
        std::move(query_doc2.filter));  // should not match any documents
    ASSERT_TRUE(writer->import(irs::directory_reader::open(data_dir, codec())));
    writer->commit();
...
for (size_t i = 10; i > 0; --i) {
  ...
  for (size_t j = 0; j < i; ++j) {
    ...
    // add different test docs (overlap to make every 3rd segment contain docs
    // from both sources)
    if (i % 3 == 1 || i % 3 == 2) {
      gen1.reset();
      write_segment(*writer, index_ref.back(), gen1);
...
TEST_P(index_test_case, import_concurrent) {
  ...
  for (auto& segment : reader) {
    ...
    while (docsItr->next()) {
      ASSERT_EQ(docsItr->value(), values->seek(docsItr->value()));
      ASSERT_EQ(1, names.erase(irs::to_string<std::string>(
                       actual_value->value.c_str())));
      ++removed;
    }
    ASSERT_FALSE(docsItr->next());
...
TEST_P(index_test_case, concurrent_consolidation) {
  ...
  auto consolidate_range = [](irs::index_writer::consolidation_t& candidates,
                              const irs::index_meta& meta, size_t begin,
                              size_t end) {
  ...
  for (size_t i = 0; i < thread_count; ++i) {
    pool.emplace_back(std::thread([&wait_for_all, &consolidate_range, &writer,
                                   i]() mutable {
      wait_for_all();
      ...
      while (num_segments > 1) {
        auto policy =
            [&consolidate_range, &i, &num_segments](
...
TEST_P(index_test_case, consolidate_invalid_candidate) {
  ...
  // invalid candidate
  {
    const irs::segment_meta meta("foo", nullptr, 6, 5, false,
                                 irs::segment_meta::file_set(),
                                 irs::field_limits::invalid());

    auto invalid_candidate_policy =
        [&meta](irs::index_writer::consolidation_t& candidates,
                const irs::index_meta& /*meta*/,
                const irs::index_writer::consolidating_segments_t&) {
...
TEST_P(index_test_case, consolidate_single_segment) {
  ...
  // single segment without deletes
  {
    ...
    ASSERT_TRUE(writer->consolidate(irs::index_utils::consolidation_policy(
        irs::index_utils::consolidate_count())));  // nothing to consolidate
    ASSERT_TRUE(writer->consolidate(
...
TEST_P(index_test_case, segment_consolidate_long_running) {
  ...
  // long running transaction + segment removal
  {
    ...
    std::thread consolidation_thread([&writer]() {
      ...
      ASSERT_FALSE(writer->consolidate(irs::index_utils::consolidation_policy(
          irs::index_utils::consolidate_count())));  // consolidate
...
TEST_P(index_test_case, consolidate_check_consolidating_segments) {
  ...
  for (size_t i = 0, j = 0; i < SEGMENTS_COUNT / 2; ++i) {
    auto merge_adjacent =
        [&j](irs::index_writer::consolidation_t& candidates,
             const irs::index_meta& meta,
             const irs::index_writer::
                 consolidating_segments_t& /*consolidating_segments*/) {
    ...
    auto check_consolidating_segments =
        [](irs::index_writer::consolidation_t& /*candidates*/,
          ...
          for (auto& segment : meta) {
            ASSERT_TRUE(consolidating_segments.end() !=
                        consolidating_segments.find(&segment.meta));
          }
        };
    ASSERT_TRUE(writer->consolidate(check_consolidating_segments));
  }
...
TEST_P(index_test_case, segment_consolidate_pending_commit) {
  ...
  // consolidate with deletes
  {
    ...
    ASSERT_EQ(3, irs::directory_cleaner::clean(
                     dir()));  // segments_2 + stale segment 1 meta  + unused
                               // column store
  ...
  // consolidate with deletes
  {
    ...
    writer->commit();  // commit pending merge
    ASSERT_EQ(count + 3, irs::directory_cleaner::clean(
                             dir()));  // +1 for segments, +1 for segment 1 doc
                                       // mask, +1 for segment 2 doc mask
  ...
  // consolidate with delete committed and pending
  {
    ...
    auto do_commit_and_consolidate_count =
        [&writer](irs::index_writer::consolidation_t& candidates,
                  const irs::index_meta& meta,
                  const irs::index_writer::consolidating_segments_t&
                      consolidating_segments) {
          auto sub_policy = irs::index_utils::consolidation_policy(
              irs::index_utils::consolidate_count());
    ...
    // check all old segments are deleted (no old version of segments is left in
    // cache and blocking )
    ASSERT_EQ(count + 3,
  ...
  // repeatable consolidation of already consolidated segment during two phase
  // commit
  {
    SetUp();
    auto query_doc1 = irs::iql::query_builder().build("name==A", "C");
    ...
    ASSERT_EQ(count + 6,
              irs::directory_cleaner::clean(
                  dir()));  // +1 for segments, +1 for segment 1 doc mask, +1
                            // for segment 1 meta, +1 for segment 2 doc mask, +1
                            // for segment 2 meta + unused column store
  ...
  // consolidate with deletes + inserts
  {
    ...
    // pending consolidation will fail (because segment 2 will have no live docs
...
TEST_P(index_test_case, segment_consolidate) {
  ...
  auto always_merge = irs::index_utils::consolidation_policy(
      irs::index_utils::consolidate_count());
  constexpr irs::IndexFeatures all_features =
      irs::IndexFeatures::FREQ | irs::IndexFeatures::POS |
      irs::IndexFeatures::OFFS | irs::IndexFeatures::PAY;
  ...
  // do not defragment old segment with uncommited removal (i.e. do not consider
  // uncomitted removals)
  {
    auto query_doc1 = irs::iql::query_builder().build("name==A", "C");
    auto writer = open_writer();
...
TEST_P(index_test_case, segment_consolidate_policy) {
  ...
  // valid segment bytes_accum policy (merge)
  {
    ...
    ASSERT_TRUE(writer->consolidate(irs::index_utils::consolidation_policy(
        options)));  // value garanteeing merge
    writer->commit();
    // segments merged because segment[0] is a candidate and needs to be merged
    // with something
  ...
  // valid docs count policy (not modified)
  {
...

    {
      ...
      ASSERT_EQ(expectedName.size() + 3,
                segment.docs_count());  // total count of documents (+3 == B, C,
                                        // D masked)
      auto terms = segment.field("same");
      ASSERT_NE(nullptr, terms);
      auto termItr = terms->iterator(irs::SeekMode::NORMAL);
...
TEST_P(index_test_case, writer_insert_immediate_remove) {
  ...
  // consolidation is needed to force opening all file handles and make cached
  // readers indeed hold reference to a file
  auto query_doc3 = irs::iql::query_builder().build("name==C", "C");
...
TEST_P(index_test_case_14, consolidate_multiple_stored_features) {
  ...
  sNumCalls.clear();
  const auto res = writer->consolidate(irs::index_utils::consolidation_policy(
      irs::index_utils::consolidate_count{}));
  ASSERT_TRUE(res);
