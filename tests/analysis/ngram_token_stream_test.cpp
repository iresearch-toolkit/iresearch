...
TEST(ngram_token_stream_test, test_load) {
  ...
  {
    std::u32string utf32data = U"\u00C0\u00C1\u00C2\u00C3\u00C4";
    auto stream = irs::analysis::analyzers::get(
      "ngram", irs::type<irs::text_format::json>::get(),
      "{ \"min\":5,\"max\":5,\"preserveOriginal\":false,\"streamType\":\"utf8\"}");

    const irs::string_ref data{
      reinterpret_cast<const char*>(utf32data.c_str()),
      utf32data.size() * sizeof(char32_t) };
  ...
  {
    std::u16string utf16data = u"\u00C0\u00C1\u00C2\u00C3\u00C4";
    auto stream = irs::analysis::analyzers::get(
      "ngram", irs::type<irs::text_format::json>::get(),
      "{ \"min\":5,\"max\":5,\"preserveOriginal\":false,\"streamType\":\"utf8\"}");

    const irs::string_ref data{
      reinterpret_cast<const char*>(utf16data.c_str()),
      utf16data.size() * sizeof(char16_t) };
