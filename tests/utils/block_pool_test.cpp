 ...
 public:
  ...
  void alloc_greedy_slice() {
    ...
    for (size_t i = 1; i < irs::detail::LEVEL_MAX; ++i) {
      ...
      ASSERT_EQ(i, *begin);
      ++begin;  // slice header
      for (size_t j = 0; j < irs::detail::LEVELS[i].size - sizeof(uint32_t) - 1;
           ++j) {
        ASSERT_EQ(0, *begin);
        ++begin;
      }
  ...
  void greedy_slice_between_blocks() {
    ...
    // write phase
    {
      ...
      ASSERT_EQ(BlockSize, slice_chain_begin);
      ASSERT_EQ(BlockSize + irs::detail::LEVELS[1].size,
                ins.pool_offset());  // level 0 makes no sense for greedy slice

      // slice should be initialized
      ASSERT_TRUE(
        std::all_of(pool_.seek(BlockSize + 1),
                    pool_.seek(BlockSize + irs::detail::LEVELS[1].size -
                               sizeof(uint32_t) - 1),
                    [](T val) { return 0 == val; }));
...
TEST_F(byte_block_pool_test, ctor) { ctor(); }
...
TEST_F(byte_block_pool_test, slice_alignment) { slice_between_blocks(); }
...
TEST_F(byte_block_pool_test, alloc_slice) { alloc_slice(); }

TEST_F(byte_block_pool_test, slice_chunked_read_write) {
  slice_chunked_read_write();
}

TEST_F(byte_block_pool_test, alloc_greedy_slice) { alloc_greedy_slice(); }
