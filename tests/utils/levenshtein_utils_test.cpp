...
void assert_description(
  ...
  for (auto& entry : candidates) {
    ...
    SCOPED_TRACE(testing::Message("Target: '")
                 << irs::ref_cast<char>(target) << "', Candidate: '"
                 << irs::ref_cast<char>(candidate)
...
void assert_description(
  ...
  const std::vector<std::tuple<irs::bytes_ref, size_t, size_t, size_t>>&
    candidates) {
  return assert_description(description, irs::EmptyBytesRef(), target,
                            candidates);
}
...
TEST(levenshtein_utils_test, test_distance) {
...

  {
    ...
    irs::utf8_utils::utf8_to_utf32<false>(irs::ref_cast<irs::byte_type>(rhs),
