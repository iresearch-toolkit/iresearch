...
TEST_F(async_utils_tests, test_thread_pool_stop_mt) {
  ...
  // test multiple calls to stop will all block
  {
    ...
    ASSERT_EQ(std::cv_status::timeout, cond2.wait_for(lock2, std::chrono::milliseconds(7000))); // assume thread blocks in 7000ms (5000ms is not enough for MSVC2015@appveyor, 6000ms is not enough for MSVC2017@jenkins)
    lock2.unlock();
    ASSERT_EQ(std::cv_status::no_timeout, cond.wait_for(lock, std::chrono::milliseconds(1000)));
    thread1.join();
    thread2.join();
