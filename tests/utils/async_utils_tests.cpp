...
TEST_F(async_utils_tests, test_thread_pool_run) {
  ...
  // test schedule 1 task exception + 1 task
  {
    ...
    pool.run(std::move(task1));
    pool.run(std::move(task2));
    ASSERT_TRUE(count || std::cv_status::no_timeout == cond.wait_for(dummy_lock, std::chrono::milliseconds(2000)) || count); // wait for all 2 tasks (exception trace is slow on MSVC)
