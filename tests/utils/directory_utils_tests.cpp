...
TEST_F(directory_utils_tests, test_reference) {
  // test clear refs (all free)
  {
    ...
    auto& attribute = const_cast<const irs::attributes&>(dir.attributes()).get<iresearch::index_file_refs>();
  ...
  // test add file (no missing)
  {
    ...
    auto& attribute = const_cast<const irs::attributes&>(dir.attributes()).get<irs::index_file_refs>();
