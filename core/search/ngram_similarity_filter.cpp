 ...
 private:
  ...
  struct search_state {
    ...
    void reversed_sequence(std::vector<const score*>& scores, std::vector<size_t>& poses) const {
      ...
      while (cur_parent) {
        scores.push_back(cur_parent->scr);
        poses.push_back(cur_parent->pos);
