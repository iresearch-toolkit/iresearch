...
const irs::score& score::no_score() noexcept { return kEmptyScore; }
...
void score::reset() noexcept {
  func_.reset(reinterpret_cast<score_ctx*>(data()), &::default_score);
}
      ...
      if (!scorer.bucket->score_offset) {
        ...
        struct ctx : score_ctx {
          explicit ctx(order::prepared::scorers::scorer&& scorer,
              ...
              : scorer(std::move(scorer)), score_buf(score_buf) {}
