...
const feature_info_provider_t kDefaultFeatureInfo = [](irs::type_info::type_id) {
  // no compression, no encryption
  ...
  for (auto& modification : modifications) {
    ...
    while (itr->next()) {
      ...
      if (modification.update
          && doc_ctx.update_id != kNonUpdateRecord
...
using candidates_mapping_t = absl::flat_hash_map<
  ...
  std::pair<
    ...
    std::pair<const segment_meta*, size_t>>>; // old segment + index within merge_writer
...
bool map_removals(
  ...
  for (auto& mapping : candidates_mapping) {
    ...
    if (segment_mapping.first->version != segment_mapping.second.first->version) {
      ...
      if (!current_itr->next()) {
        do {
          assert(doc_limits::valid(merge_ctx.doc_map(merged_itr->value()))); // doc_id must have a valid mapping
...
index_writer::flush_context_ptr index_writer::documents_context::update_segment(
  ...
  if (writer.initialized() && !no_flush) {
...
index_writer::segment_context::ptr index_writer::segment_context::make(
  ...
  return memory::make_unique<segment_context>(
    ...
    column_info, feature_info, comparator);
}
...
index_writer::ptr index_writer::make(
  ...
  PTR_NAMED(
    ...
    opts.comparator,
    opts.column_info
      ? opts.column_info : kDefaultColumnInfo,
    opts.features
      ? opts.features : kDefaultFeatureInfo,
...
index_writer::consolidation_result index_writer::consolidate(
  ...
  // commit merge
  {
    ...
    if (pending_state_) {
      ...
      auto unregister_missing_cached_readers = make_finally(std::move(cleanup_cached_readers));
...
index_writer::active_segment_context index_writer::get_segment_context(
  ...
  auto segments_active_decrement = make_finally([this]()noexcept{ --segments_active_; });
  ...
  if (segment_memory_max &&
    ...
    segment_ctx->writer_ = segment_writer::make(
      segment_ctx->dir_,  column_info_, feature_info_, comparator_);
  }
