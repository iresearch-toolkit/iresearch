...
void write_offset(posting& p, Stream& out, const uint32_t base,
                  const offset& offs) {
 ...
 public:
  ...
  void reset(IndexFeatures features, const frequency& freq) {
    ...
    std::get<attribute_ptr<payload>>(attrs_) =
        IndexFeatures::NONE != (features & IndexFeatures::PAY) ? &pay_
                                                               : nullptr;
  }
...
field_data::field_data(string_ref name, features_t features,
