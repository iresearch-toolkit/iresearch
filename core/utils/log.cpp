...
void fd_log_appender(void* context, char const* function, char const* file,
  ...
  if (fd != nullptr) {
    const auto* prefix =
      level < std::size(LOG_LEVELS) ? LOG_LEVELS[level] : "UNKNOWN";
    std::fprintf(fd, "%s: %s:%d %s\n", prefix,
                 (file ? file : "<no file provided>"), line,
...
class logger_ctx : public irs::singleton<logger_ctx> {
 public:
  logger_ctx()
    : singleton(), stack_trace_level_(irs::logger::level_t::IRL_DEBUG) {
  ...
  bool enabled(irs::logger::level_t level) const {
    return noop_log_appender != out_[level].load().appender_;
  }

  logger_ctx& output(irs::logger::level_t level,
                     irs::logger::log_appender_callback_t appender,
                     void* context) {
    if (appender != nullptr) {
  ...
  logger_ctx& output_le(irs::logger::level_t level,
                        irs::logger::log_appender_callback_t appender,
                        void* context) {
    for (size_t i = 0, count = std::size(out_); i < count; ++i) {
      output(static_cast<irs::logger::level_t>(i),
             i > level ? nullptr : appender, context);
    }
    return *this;
  }
  ...
  void log(const char* function, const char* file, int line,
    ...
    log_ctx.appender_(log_ctx.appender_context_, function, file, line, level,
                      message, len);
  }
 ...
 private:
  struct alignas(IRESEARCH_CMPXCHG16B_ALIGNMENT) level_ctx_t {
    ...
    level_ctx_t() noexcept
      : appender_(noop_log_appender), appender_context_(nullptr) {}
    level_ctx_t(irs::logger::log_appender_callback_t appender,
  ...
  std::atomic<level_ctx_t>
    out_[irs::logger::IRL_TRACE +
         1];  // IRL_TRACE is the last value, +1 for 0'th id
...
DWORD stack_trace_win32(irs::logger::level_t level,
  ...
  if (!ex || !ex->ContextRecord) {
    IR_LOG_FORMATED(
      level, "No stack_trace available");  // cppcheck-suppress syntaxError
  ...
  if (!EnumProcessModules(process, &module_handle, sizeof(HMODULE),
                          &module_handle_size)) {
  ...
  GetModuleInformation(process, module_handle, &module_info,
                       sizeof(MODULEINFO));
  ...
  while (StackWalk64(image_type, process, thread, &frame, &ctx, NULL, NULL,
    ...
    auto has_line = SymGetLineFromAddr(process, frame.AddrPC.Offset,
    ...
    if (skip_frame) {
      if (has_symbol && stack_trace_fn_symbol == symbol.Name) {
        skip_frame =
          false;  // next frame is the start of the exception stack trace
      }
    ...
    if (has_line) {
      ss << ": " << line.FileName << ":" << std::dec << line.LineNumber << "+0x"
         << std::hex << offset_from_line;
    }
...
bool file_line_addr2line(irs::logger::level_t level, const char* obj,
  ...
  if (!pid) {
    char name_buf[PROC_PIDPATHINFO_MAXSIZE];  // buffer size requirement for
                                              // proc_pidpath(...)
    auto ppid = getppid();

    if (0 < proc_pidpath(ppid, name_buf, sizeof(name_buf))) {
      // The exec() family of functions replaces the current process image with
      // a new process image. The exec() functions only return if an error has
      // occurred.
...
bool file_line_addr2line(irs::logger::level_t level, const char* obj,
  ...
  if (!pid) {
    constexpr const size_t pid_size =
      sizeof(pid_t) * 3 + 1;  // aproximately 3 chars per byte +1 for \0
    constexpr const char PROC_EXE[] = {"/proc//exe"};
    constexpr const size_t name_size =
      std::size(PROC_EXE) + pid_size + 1;  // +1 for \0
    ...
    // new process image. The exec() functions only return if an error has
    // occurred.
    dup2(fd, 1);  // redirect stdout to fd
    dup2(fd, 2);  // redirect stderr to fd
...
bool file_line_bfd(const bfd_callback_type_t& callback, const char* obj,
  ...
  return address < 0 || suffix ? false
                               : file_line_bfd(callback, obj, (void*)address);
}

std::shared_ptr<char> proc_name_demangle(const char* symbol) {
  int status;

  // abi::__cxa_demangle(...) expects 'output_buffer' to be malloc()ed and does
  // realloc()/free() internally
...
bool stack_trace_gdb(irs::logger::level_t level, int fd) {
  ...
  if (!pid) {
    constexpr const size_t pid_size =
      sizeof(pid_t) * 3 + 1;  // approximately 3 chars per byte +1 for \0
    char pid_buf[pid_size];
    char name_buf[PROC_PIDPATHINFO_MAXSIZE];  // buffer size requirement for
                                              // proc_pidpath(...)
    auto ppid = getppid();
    ...
    if (0 < proc_pidpath(ppid, name_buf, sizeof(name_buf))) {
      ...
      dup2(fd, 1);  // redirect stdout to fd
      dup2(fd, 2);  // redirect stderr to fd
      execlp("gdb", "gdb", "-n", "-nx", "-return-child-result", "-batch", "-ex",
             "thread", "-ex", "bt", name_buf, pid_buf, NULL);
    }
...
bool stack_trace_gdb(irs::logger::level_t level, int fd) {
  ...
  if (!pid) {
    ...
    dup2(fd, 2);  // redirect stderr to fd
    execlp("gdb", "gdb", "-n", "-nx", "-return-child-result", "-batch", "-ex",
           "thread", "-ex", "bt", name_buf, pid_buf, nullptr);
    exit(1);
  }
...
void stack_trace_posix(irs::logger::level_t level, int out_pipe) {
  ...
  if (out == nullptr) {
    IR_LOG_FORMATED(level,
                    "Failed to open pipe. Outputting stack trace to stderr");
    out = stderr;
  }
  ...
  auto frames_buf_ptr =
    frames_buf + 2;  // +2 to skip backtrace(...) + stack_trace_posix(...)
  int pipefd[2];

  if (pipe(pipefd)) {
    IR_LOG_FORMATED(level,
                    "Failed to output detailed stack trace to stream, "
                    "outputting plain stack trace");
    backtrace_symbols_fd(frames_buf_ptr, frames_count,
                         fileno(stderr));  // fallback plain output
    return;
  }
      ...
      if (path_start < path_end) {
        if (offset_start < offset_end) {
          ...
          if (fn_start < fn_end) {
            ...
            *path_end = '\0';
            ...
            if (!file_line_bfd(callback, path_start, offset_start) &&
                !file_line_addr2line(level, path_start, offset_start,
                                     out_pipe)) {
              std::fprintf(out, "\n");
              std::fflush(out);
            }
        ...
        if (addr_start < addr_end) {
          ...
          *path_end = '\0';
          ...
          if (!file_line_bfd(callback, path_start, addr_start) &&
              !file_line_addr2line(level, path_start, addr_start, out_pipe)) {
            std::fprintf(out, "\n");
            std::fflush(out);
          }
...
bool file_line_bfd(const bfd_callback_type_t& callback, const char* obj,
  ...
  constexpr const size_t symbols_size =
    1048576;  // arbitrary size (4K proved too small)
  ...
  if (!bfd_find_nearest_line(file_bfd, section, symbols_ptr, offset, &file,
                             &func, &line)) {
...
bool file_line_addr2line(irs::logger::level_t level, const char* obj,
                         unw_word_t addr, int fd) {
  size_t addr_size = sizeof(unw_word_t) * 3 + 2 +
                     1;  // approximately 3 chars per byte +2 for 0x, +1 for \0
...
bool stack_trace_libunwind(irs::logger::level_t level, int output_pipe) {
  ...
  while (unw_step(&cursor) > 0) {
    ...
    if (0 != dladdr((void*)instruction_pointer, &dl_info) ||
        !dl_info.dli_fname) {
      // there appears to be a magic number base address which should not be
      // subtracted from the instruction_pointer
      static const void* static_fbase = (void*)0x400000;
      auto addr = instruction_pointer - (static_fbase == dl_info.dli_fbase
                                           ? unw_word_t(dl_info.dli_saddr)
                                           : unw_word_t(dl_info.dli_fbase));
      bool use_addr2line = false;
      bfd_callback_type_t callback =
        [level, out, instruction_pointer, &addr, &dl_info, &use_addr2line,
         output_pipe](const char* file, size_t line, const char* fn) -> void {
        std::fprintf(out, "%s(",
        ...
        std::fprintf(out, "+0x%lx)[0x%lx] ",
                     dl_info.dli_saddr
                       ? (instruction_pointer - unw_word_t(dl_info.dli_saddr))
                       : addr,
                     instruction_pointer);
    ...
    std::fprintf(out, "\?\?(%s+0x%lx)[0x%lx]\n",
                 proc_name ? proc_name.get() : proc_buf, offset,
                 instruction_pointer);
    std::fflush(out);
  }
  return true;
...
#else
bool stack_trace_libunwind(irs::logger::level_t, int) { return false; }
#endif
...
bool enabled(level_t level) { return logger_ctx::instance().enabled(level); }
 ...
 public:
  int start(level_t level) {
    ...
    if (!pipes_opened_) {
      IR_LOG_FORMATED(level,
                      "Failed to output detailed stack trace to stream, "
                      "outputting plain stack trace to stderr");
...
void stack_trace(level_t level) {
  ...
  try {
    ...
    printer.stop();
    stack_trace_nomalloc(level, printer.start(level),
                         2);  // +2 to skip stack_trace()
    throw;
  }
#endif
}
