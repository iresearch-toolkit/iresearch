...
class compact<I, T, false> {
 public:
  ...
  compact() = default;
  compact(const compact& rhs) noexcept : val_(rhs.val_) {}
  compact(compact&& rhs) noexcept : val_(std::move(rhs.val_)) {}

  template<typename U = T>
  explicit compact(U&& value) noexcept : val_(std::forward<U>(value)) {
...
class compact_ref<I, T, false> {
 public:
  ...
  template<typename U = T>
  explicit compact_ref(U& value) noexcept : val_(&value) {}
...
class compact_pair : private compact<0, T0>, private compact<1, T1> {
 public:
  ...
  const first_type& first() const noexcept { return first_compressed_t::get(); }

  first_type& first() noexcept { return first_compressed_t::get(); }

  const second_type& second() const noexcept {
    return second_compressed_t::get();
  }

  second_type& second() noexcept { return second_compressed_t::get(); }
};  // compact_pair
