...
class IRESEARCH_API thread_pool {
 public:
  ...
  explicit thread_pool(
    ...
    size_t max_idle = 0,
    basic_string_ref<native_char_t> = basic_string_ref<native_char_t>::EMPTY);
  ~thread_pool();
  size_t max_idle();
 ...
 private:
  ...
  void worker(const thread_name_t prefix);
  ...
  std::basic_string<native_char_t> worker_prefix_;
