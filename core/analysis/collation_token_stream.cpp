...
std::array<std::vector<uint8_t>, 256> kByteRecalc{
    ...
    {0x7d},       {0x7e},       {0x7f},       {0xd0, 0x80}, {0xd0, 0x81},
    {0xd0, 0x82}, {0xd0, 0x83}, {0xd0, 0x84}, {0xd0, 0x85}, {0xd0, 0x86},
    {0xd0, 0x87}, {0xd1, 0x80}, {0xd1, 0x81}, {0xd1, 0x82}, {0xd1, 0x83},
    {0xd1, 0x84}, {0xd1, 0x85}, {0xd1, 0x86}, {0xd1, 0x87}, {0xd2, 0x80},
    {0xd2, 0x81}, {0xd2, 0x82}, {0xd2, 0x83}, {0xd2, 0x84}, {0xd2, 0x85},
    {0xd2, 0x86}, {0xd2, 0x87}, {0xd3, 0x80}, {0xd3, 0x81}, {0xd3, 0x82},
    {0xd3, 0x83}, {0xd3, 0x84}, {0xd3, 0x85}, {0xd3, 0x86}, {0xd3, 0x87},
    {0xd4, 0x80}, {0xd4, 0x81}, {0xd4, 0x82}, {0xd4, 0x83}, {0xd4, 0x84},
    {0xd4, 0x85}, {0xd4, 0x86}, {0xd4, 0x87}, {0xd5, 0x80}, {0xd5, 0x81},
    {0xd5, 0x82}, {0xd5, 0x83}, {0xd5, 0x84}, {0xd5, 0x85}, {0xd5, 0x86},
    {0xd5, 0x87}, {0xd6, 0x80}, {0xd6, 0x81}, {0xd6, 0x82}, {0xd6, 0x83},
    {0xd6, 0x84}, {0xd6, 0x85}, {0xd6, 0x86}, {0xd6, 0x87}, {0xd7, 0x80},
    {0xd7, 0x81}, {0xd7, 0x82}, {0xd7, 0x83}, {0xd7, 0x84}, {0xd7, 0x85},
    {0xd7, 0x86}, {0xd7, 0x87}, {0xd8, 0x80}, {0xd8, 0x81}, {0xd8, 0x82},
    {0xd8, 0x83}, {0xd8, 0x84}, {0xd8, 0x85}, {0xd8, 0x86}, {0xd8, 0x87},
    {0xd9, 0x80}, {0xd9, 0x81}, {0xd9, 0x82}, {0xd9, 0x83}, {0xd9, 0x84},
    {0xd9, 0x85}, {0xd9, 0x86}, {0xd9, 0x87}, {0xda, 0x80}, {0xda, 0x81},
    {0xda, 0x82}, {0xda, 0x83}, {0xda, 0x84}, {0xda, 0x85}, {0xda, 0x86},
    {0xda, 0x87}, {0xdb, 0x80}, {0xdb, 0x81}, {0xdb, 0x82}, {0xdb, 0x83},
    {0xdb, 0x84}, {0xdb, 0x85}, {0xdb, 0x86}, {0xdb, 0x87}, {0xdc, 0x80},
    {0xdc, 0x81}, {0xdc, 0x82}, {0xdc, 0x83}, {0xdc, 0x84}, {0xdc, 0x85},
    {0xdc, 0x86}, {0xdc, 0x87}, {0xdd, 0x80}, {0xdd, 0x81}, {0xdd, 0x82},
    {0xdd, 0x83}, {0xdd, 0x84}, {0xdd, 0x85}, {0xdd, 0x86}, {0xdd, 0x87},
    {0xde, 0x80}, {0xde, 0x81}, {0xde, 0x82}, {0xde, 0x83}, {0xde, 0x84},
    {0xde, 0x85}, {0xde, 0x86}, {0xde, 0x87}, {0xdf, 0x80}, {0xdf, 0x81},
    {0xdf, 0x82}, {0xdf, 0x83}, {0xdf, 0x84}, {0xdf, 0x85}, {0xdf, 0x86},
    {0xdf, 0x87}};
...
bool collation_token_stream::reset(string_ref data) {
  ...
  int32_t term_size = state_->collator->getSortKey(
    icu_token, raw_term_buf, sizeof raw_term_buf);
  ...
  --term_size;
  assert(0 == raw_term_buf[term_size]);
  ...
  for (size_t i = 0; i < term_size; ++i) {
    auto const& encoded = kByteRecalc[raw_term_buf[i]];
    for (auto b : encoded) {
      if (termBufIdx > sizeof state_->term_buf) {
        IR_FRMT_ERROR(
            "Collated token is more than %d bytes length after encoding.",
            static_cast<int32_t>(sizeof state_->term_buf));
        return false;
      }
      state_->term_buf[termBufIdx++] = b;
    }
  }

  std::get<term_attribute>(attrs_).value = {
    state_->term_buf, static_cast<size_t>(termBufIdx) };
