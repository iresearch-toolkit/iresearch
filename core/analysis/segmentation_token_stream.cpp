...
const frozen::set<uint32_t, 10> WHITE_SPACE_TABLE = {
...
constexpr char char_primary_category(uint32_t c) noexcept {
  return char(uint16_t(char_general_category(c)) >> 8);
}

constexpr bool char_is_alphanumeric(uint32_t c) noexcept {
  auto g = char_primary_category(c);
...
using word_break_t = irs::analysis::segmentation_token_stream::options_t::word_break_t;
...
bool accept_token(Iterator begin, Iterator end, word_break_t wb) {
  switch (wb) {
    ...
    case word_break_t::GRAPHIC:
      return std::find_if_not(begin, end, char_is_white_space) != end;
    case word_break_t::ALPHA:
      return std::find_if(begin, end, char_is_alphanumeric) != end;
    default:
      assert(false);
      return false;
  }
  ...
  while (true) {
    ...
    switch (options_.case_convert) {
      ...
      case options_t::case_convert_t::LOWER:
        ...
        std::get<2>(attrs_).value =
           irs::ref_cast<irs::byte_type>(term_buf_);
        break;
...
bool segmentation_token_stream::reset(const string_ref& data) {
  state_->data.assign(data.begin(), data.end());
