...
bool delimited_token_stream::next() {
  ...
  offset_.start = uint32_t(std::min(
    size_t(irs::integer_traits<uint32_t>::const_max),
    offset_.end + delim_.size()
  )); // if value > max then set it to max
  offset_.end = uint32_t(std::min(
    size_t(irs::integer_traits<uint32_t>::const_max),
    offset_.start + size
  )); // if value > max then set it to max
...
bool delimited_token_stream::reset(const string_ref& data) {
  ...
  offset_.start = 0;
  offset_.end = // if value > max then set it to max as a counterpart to computation in next() above
    0 - uint32_t(std::min(size_t(irs::integer_traits<uint32_t>::const_max), delim_.size()));
